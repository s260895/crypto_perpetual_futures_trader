...
def generate_signal(current_input=None,strat='ema_cross_over_under',fast_ema=10,slow_ema=40):
    if current_input is None:
        return "NO INPUT"  
    min_input_length =   np.max([float(fast_ema),float(slow_ema)])
...
def get_balance(mode='paper',asset='USDT'):
    ...
    if mode == 'live':
        ...
        balance = {'wallet_balance': str(float([float(elem['balance']) for elem in exchange.fapiPrivate_get_balance() if elem['asset']==asset][0])),
...
def runner(mode='paper',symbol='BTC/USDT',n=50,interval='1h',asset='USDT',strat='ema_cross_over_under',fast_ema=10,slow_ema=40,short_leverage="2",long_leverage="4"):
    ...
    while True:
        ...
        try:
            ...
            if datetime.datetime.utcnow().minute in check_minutes and  datetime.datetime.utcnow().hour in check_hours:
                ...
                print("Unrealized PnL = "+balance['unrealized_pnl'])
